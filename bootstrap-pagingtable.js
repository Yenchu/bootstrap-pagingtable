...
+function($) { "use strict";
  ...
  PagingTable.DEFAULTS = {
    ...
    , texts: {
      ...
      , cancelButton:'Cancel'}
    , pagerTemplate: '' 
      + '<span class="row"><span class="col-md-1"><span class="input-group input-group-sm">'
      + '<span class="input-group-addon" style="background-color:#fff;border:0">View {{fromRecord}} - {{toRecord}} of {{totalRecords}} </span>'
      + '<span class="input-group-btn dropup"><a class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown">{{pageSize}} <span class="caret"></span></a>'
      + '<ul class="dropdown-menu page-size-options" role="menu"></ul></span>'
      + '<span class="input-group-addon" style="background-color:#fff;border:0"> per page</span></span></span>'
      + '<span class="col-md-offset-9 col-md-2"><span class="pull-right input-group input-group-sm table-paging-btns">'
      + '<span class="input-group-btn">{{firstButton}}{{prevButton}}</span>'
  ...
  PagingTable.prototype.init = function(element, options) {
    ...
    this.pagingBtnsClassName = 'table-paging-btns';
    this.$element = $(element);
    this.$pagerParent = this.$element;//.parent();
  ...
  PagingTable.prototype.createPager = function() {
    ...
    if (pagerElemName === 'thead') {
      $pager.prependTo(this.$pagerParent);
    } else {
      $pager.appendTo(this.$pagerParent);
    }
    
    var colspan = this.getColSpan();
    var tr = '<tr><td colspan="' + colspan + '">' + this.getPagerContent(this.options.pagerTemplate) + '</td></tr>';
    $pager.html(tr);
  ...
  PagingTable.prototype.getPagerContent = function(tpl) {
    ...
    tpl = tpl.replace('{{pageSize}}', '<span class="page-size-value"></span>');
      
    tpl = tpl.replace('{{firstButton}}', this.options.firstButtonTemplate);
    tpl = tpl.replace('{{prevButton}}', this.options.prevButtonTemplate);
    
    tpl = tpl.replace('{{currentPage}}', this.options.currentPageTemplate);
    tpl = tpl.replace('{{totalPages}}', '<span class="total-pages"></span>');
    
    tpl = tpl.replace('{{nextButton}}', this.options.nextButtonTemplate);
    tpl = tpl.replace('{{lastButton}}', this.options.lastButtonTemplate);
    return tpl;
  };
  ...
  PagingTable.prototype.setPageSizeElement = function() {
    ...
    this.$pagerParent.find('.page-size-options').html(sizeOptions);
  };
  ...
  PagingTable.prototype.addEventHandlers = function() {
    ...
    $(document).off('click.' + ns).on('click.' + ns, function(e) {
      ...
      $(this).find('table').each(function() {
        var comp = $(this).data(compName);
        if (typeof comp !== 'undefined' && comp.namespace === compName) {
          if (comp.isBlur(e)) {
            comps.push(comp);
          }
        }
      ...
      for (var i = 0; i < comps.length; i++) {
        var comp = comps[i];
        var be = $.Event('blur');
        comp.$element.trigger(be);

        // unselect rows if unselectByBlur enabling
        if (comp.options.unselectByBlur) {
          var rids = comp.getSelectedRowIds();
          comp.removeSelectedRows();
          comp.triggerUnselectRowEvent(rids);
        }
      }
      comps = null;
    });
    ...
    $(document).off('keyup.' + ns).on('keyup.' + ns, function(e) {
      ...
      $(this).find('table').each(function() {
        var comp = $(this).data(compName);
        if (typeof comp !== 'undefined' && comp.namespace === compName) {
          if (comp.options.inlineEditing && comp.editedRowId) {
            if (key === 13) { // Enter key
              comp.saveRow(comp.editedRowId);
            } else if (key === 27) { // Esc key
              comp.restoreRow(comp.editedRowId);
            }
          }
        }
      });
    ...
    $element.off('click.tr.' + ns).on('click.tr.' + ns, 'tbody tr', function(e) {
      ...
      if (!that.options.selectByCheckbox) {
        var $this = $(this);
        if (!that.isSelectedRow($this)) {
          that.options.isMultiSelect ? that.selectRows($this, e) : that.selectRow($this);
          that.triggerSelectRowEvent(that.getRowId($this));
        } else {
          if (!that.options.unselectByBlur) {
            // unselect row by clicking it again
            that.removeSelectedRow($this);
            that.triggerUnselectRowEvent(that.getRowId($this));
          } else {
            // clear previous selected row(s) and reselect this row
            if (that.options.isMultiSelect) {
              var rids = that.getSelectedRowIds();
              that.triggerUnselectRowEvent(rids);
              that.selectRow($this);
              that.triggerSelectRowEvent(that.getRowId($this));
            }
          }
    ...
    $element.off('mouseenter.tr.' + ns).on('mouseenter.tr.' + ns, 'tbody tr', function() {
      $(this).addClass(that.options.classes.hover);
    });
    $element.off('mouseleave.tr.' + ns).on('mouseleave.tr.' + ns, 'tbody tr', function() {
      $(this).removeClass(that.options.classes.hover);
    });
    ...
    if (options.selectByCheckbox) {
      if (options.isMultiSelect) {
        $element.off('change.select-all.' + ns).on('change.select-all.' + ns, '.select-all', function(e) {
          ...
          if (checked) {
            ...
            var rids = that.getSelectedRowIds();
            that.triggerSelectRowEvent(rids);
          } else {
            var rids = that.getSelectedRowIds();
      ...
      $element.off('change.select-one.' + ns).on('change.select-one.' + ns, '.select-one', function(e) {
        ...
        if (checked) {
          ...
          that.triggerSelectRowEvent(that.getRowId($tr));
        } else {
          that.removeSelectedRow($tr);
          that.triggerUnselectRowEvent(that.getRowId($tr));
        }
      });
    }
  };
  ...
  PagingTable.prototype.addPagingEventHandlers = function() {
    var that = this, $pagerParent = this.$pagerParent, ns = this.namespace;
    
    $pagerParent.off('click.first-page.' + ns).on('click.first-page.' + ns, '.first-page', function() {
      if (that.page <= 0) {
        return;
      }
      that.page = 0;
    ...
    $pagerParent.off('click.prev-page.' + ns).on('click.prev-page.' + ns, '.prev-page', function() {
      if (that.page <= 0) {
        return;
      }
      that.page -= 1;
    ...
    $pagerParent.off('click.next-page.' + ns).on('click.next-page.' + ns, '.next-page', function() {
    ...
    $pagerParent.off('click.last-page.' + ns).on('click.last-page.' + ns, '.last-page', function() {
      if (that.page >= that.totalPages - 1) {
        return;
      }
    ...
    $pagerParent.off('keyup.page-changed.' + ns).on('keyup.page-changed.' + ns, '.current-page', function(e) {
      e.stopPropagation();
      var key = e.charCode || e.keyCode;
      if (key === 13) {
        var pageNo = parseInt($pagerParent.find('.current-page').val());
        if (!pageNo || pageNo === that.page + 1) {
          $pagerParent.find('.current-page').val((that.page + 1));
          return;
        }
    ...
    $pagerParent.off('click.page-size-changed.' + ns).on('click.page-size-changed.' + ns, '.page-size-options li a', function() {
  ...
  PagingTable.prototype.updatePagingElements = function() {
    var $pagerParent = this.$pagerParent;
    var page = this.page, totalPages = this.totalPages, pageSize = this.pageSize, totalRecords = this.totalRecords;
    $pagerParent.find('.current-page').val((page + 1));
    $pagerParent.find('.total-pages').text(totalPages);
    
    var fromRecord = page * pageSize + 1;
    var toRecord = fromRecord + pageSize - 1;
    toRecord = toRecord >= totalRecords ? totalRecords : toRecord;
    $pagerParent.find('.from-record').text(fromRecord);
    $pagerParent.find('.to-record').text(toRecord);
    $pagerParent.find('.total-records').text(totalRecords);
    
    $pagerParent.find('.page-size-value').text(pageSize);
  ...
  PagingTable.prototype.load = function() {
    ...
    if (rowLen > 0) {
      ...
      var $pager = this.$pagerParent.find('.' + this.pagerClassName);
      $pager.hasClass('hide') && $pager.removeClass('hide');
      
      var $pagingBtns = $pager.find('.table-paging-btns');
  ...
  PagingTable.prototype.createRowContent = function(rowData, woTr) {
    ...
    for (var j = 0, colLen = this.colModels.length; j < colLen; j++) {
  ...
  PagingTable.prototype.triggerSelectRowEvent = function(rowId) {
    var rowIds = $.isArray(rowId) ? rowId : [rowId];
    if (this.options.isMultiSelect) {
      this.$element.trigger({type:'selectRow', rowId:rowIds[rowIds.length - 1], rowIds:rowIds});
    } else {
      this.$element.trigger({type:'selectRow', rowId:rowIds[0]});
    }
  };
  
  PagingTable.prototype.triggerUnselectRowEvent = function(rowId) {
    var rowIds = $.isArray(rowId) ? rowId : [rowId];
    if (this.options.isMultiSelect) {
      this.$element.trigger({type:'unselectRow', rowId:rowIds[rowIds.length - 1], rowIds:rowIds});
    } else {
      this.$element.trigger({type:'unselectRow', rowId:rowIds[0]});
    }
  };
  ...
  PagingTable.prototype.removeSelectedRows = function() {
    ...
    if (this.options.selectByCheckbox) {
      this.options.isMultiSelect && this.$element.find('.select-all').prop('checked', false);
    }
  ...
  PagingTable.prototype.isBlur = function(e) {
    ...
    //console.log('xmin=' + xmin + ', ymin=' + ymin + ', xmax=' + xmax + ', ymax=' + ymax + ', e.pageX=' + e.pageX + ', e.pageY=' + e.pageY);
